...
function inject (bot, { physicsEnabled }) {
    ...
    function sendPacketPosition (position, onGround) {
        // sends data, no logic
        const oldPos = new Vec3(bot.lastSent.x, bot.lastSent.y, bot.lastSent.z)
        bot.lastSent.x = position.x
        bot.lastSent.y = position.y
        bot.lastSent.z = position.z
        bot.lastSent.onGround = onGround
        bot._client.write('position', bot.lastSent)
    ...
    function sendPacketLook (yaw, pitch, onGround) {
        // sends data, no logic
        const oldPos = new Vec3(bot.lastSent.x, bot.lastSent.y, bot.lastSent.z)
        bot.lastSent.yaw = yaw
        bot.lastSent.pitch = pitch
        bot.lastSent.onGround = onGround
        bot.lastRollEvents.yaw = bot.lastSentYaw
        bot.lastRollEvents.pitch = bot.lastSentPitch
        bot._client.write('look', bot.lastSent)
        bot.emit('move', oldPos)
        bot.emit('look', bot.lastRollEvents)
    }
    ...
    function sendPacketPositionAndLook (position, yaw, pitch, onGround) {
        ...
        bot.lastRollEvents.yaw = bot.lastSentYaw
        bot.lastRollEvents.pitch = bot.lastSentPitch
        bot._client.write('position_look', bot.lastSent)
        bot.emit('move', oldPos)
        bot.emit('look', bot.lastRollEvents)
    }
    ...
    function updatePosition (dt) {
        ...
        const positionUpdated = bot.lastSent.x !== position.x || bot.lastSent.y !== position.y || bot.lastSent.z !== position.z
        const lookUpdated = bot.lastSent.yaw !== yaw || bot.lastSent.pitch !== pitch
        ...
        } else if (performance.now() - bot.lastSent.time >= 1000) {
            // Send a position packet every second, even if no update was made
            sendPacketPosition(position, onGround)
            bot.lastSent.time = performance.now()
